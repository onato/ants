...
fn update_pheromone_grid(
    ...
    for position in ant_query.iter() {
        ...
        let grid_x = ((position.position.x + window_width / 2.0) as usize)
            .clamp(0, pheromone_grid.width.saturating_sub(1));
        let grid_y = ((position.position.y + window_height / 2.0) as usize)
            .clamp(0, pheromone_grid.height.saturating_sub(1));
        
        // Increase pheromone level at this position
        pheromone_grid.grid[grid_x][grid_y] += 0.1; // Increased for better visibility
...
fn update_pheromone_texture(
    ...
    // Skip if texture handle is not set yet
    if let Some(texture_handle) = &pheromone_grid.texture_handle {
